...
@section('content')
    <section class="max-w-2xl mx-auto pt-10">
        ...
        <div class="p-8 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md shadow-2xl">
            <form action="/dashboard/incomes/send" method="POST" class="flex flex-col gap-6">
                @csrf

                <div class="flex flex-col gap-2">
                    <label for="amount" class="text-xs uppercase tracking-widest text-gray-400 ml-1">Amount (R$)</label>
                    <div class="relative">
                        <span class="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-400 font-bold text-xl">R$</span>
                        <input type="number" step="0.01" name="amount" id="amount" placeholder="0,00" required
                               class="w-full pl-14 pr-5 py-5 bg-[#080616] border border-white/10 rounded-2xl text-3xl font-bold text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all placeholder:text-gray-700">
                    </div>
                </div>

                <div class="flex flex-col gap-2">
                    <label for="description" class="text-xs uppercase tracking-widest text-gray-400 ml-1">Description</label>
                    <input type="text" name="description" id="description" placeholder="Ex: Freelance Project, Salary..." required
                ...
                <div class="mt-4">
                    <button type="submit"
                            class="w-full py-5 bg-emerald-500 hover:bg-emerald-400 text-[#080616] font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98]">
                        Save Transaction
                    </button>
